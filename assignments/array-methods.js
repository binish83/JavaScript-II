...
runners.forEach(eachRunner => {
    fullName.push(`${eachRunner.first_name} ${eachRunner.last_name}`)
})
console.log(fullName);

// ==== Challenge 2: Use .map() ====
// The event director needs to have all the runner's first names converted to uppercase because the director BECAME DRUNK WITH POWER. Convert each first name into all caps and log the result
let allCaps = runners.map(mappedEachRunner => {
...
let largeShirts = runners.filter(Lshirts => {
    let shirtSize = Lshirts.shirt_size === 'L'
    return shirtSize
});
...
let ticketPriceTotal = runners.reduce((donationCount, Tallied) => {
    console.log(donationCount)
    console.log(Tallied)
    return Tallied.donation + donationCount
}, 0);
